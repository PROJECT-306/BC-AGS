        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-black overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-black border-b border-gray-200">
                    <h3 class="text-xl font-bold mb-4 text-white">Edit Class Section</h3>

                    <form method="POST" action="{{ route('class-sections.update', $classSection->class_section_id) }}">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label class="block text-white mb-1">Class Section Name</label>
                            <input type="text" name="class_section_name" value="{{ $classSection->class_section_name }}" class="w-full rounded-md p-2" required>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="user_id" class="w-full rounded-md p-2" required>
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ $user->id == $classSection->user_id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="academic_year_id" class="w-full rounded-md p-2" required>
                                ...
                                @foreach ($academicYears as $year)
                                    <option value="{{ $year->academic_year_id }}" {{ $year->academic_year_id == $classSection->academic_year_id ? 'selected' : '' }}>
